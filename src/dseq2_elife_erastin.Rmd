...
## rownames
counts %<>% set_rownames(counts$gene) %>% dplyr::select(-gene)
...
counts.sub <- counts %>% dplyr::select(-bme_1, -bme_2, -erastin_bme_1, -erastin_bme_2)
...
## shrunken lfc
resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_erastin_vs_cntrl", type="apeglm")
plotMA(resLFC, ylim=c(-2,2), main = 'MA-plot Shrunken LFCs')

# PCA
dat <- as.data.frame(t(counts))
dat$group <- factor(rep(c("Cntrl", 'BME+', "Erastin+", 'BME+Erastin+'), each = 2), levels = c("Cntrl", "Erastin+", 'BME+', 'BME+Erastin+'))
...
## Modify results format for future use
```{r}
