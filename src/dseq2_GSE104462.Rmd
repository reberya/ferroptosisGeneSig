...
# All samples 
Check to see which gives more differentially expressed genes - using all samples or using only the samples being compared. All samples will be looked at first
...
txi.kallisto.all <- tximport(files, type = "kallisto", tx2gene = tx2gene)
...
rownames(sampleTable) <- colnames(txi.kallisto.all$counts)

# import into DESeq2
dds <- DESeqDataSetFromTximport(txi.kallisto.all, sampleTable, ~condition)

# Run DESeq2
dds <- DESeq(dds)
```

## Differential expression analysis of erastin vs control using all available samples
```{r}
...
# 6 samples being compared 
Check to see which gives more differentially expressed genes - using all samples or using only the samples being compared. 
...
# stratify for 6 samples being used
files <- files[1:6]
...
# create sample table for import from tximport
sampleTable <- data.frame(condition = factor(rep(c("Hepg2", "Hepg2_erastin"), each = 3)))
...
More DEGs when just using the erastin/control samples than when including the ferrostatin samples in the analysis
```{r}
...
hist(res$pvalue, main = 'GSE104462 p-values')
...
## shrunken lfc
resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_Hepg2_erastin_vs_Hepg2", type="apeglm")
plotMA(resLFC, ylim=c(-2,2), main = 'MA-plot Shrunken LFCs')

# PCA
dat <- as.data.frame(t(txi.kallisto.all$counts))
dat$group <- factor(rep(c("Cntrl", "Erastin+", 'Ferrostatin+'), each = 3), levels = c("Cntrl", "Erastin+", 'Ferrostatin+'))
