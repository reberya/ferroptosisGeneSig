---
title: "eda_TCGA_Prostate_cancer_ferroptosis"
...
# fpkm
fpkm <- fread('/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/data/ucsc_xena/datasets/GDC_TCGA_Prostate_cancer_PRAD/TCGA-PRAD.htseq_fpkm.tsv', data.table = F)

# phenotypic data
pheno <- fread('/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/data/ucsc_xena/datasets/GDC_TCGA_Prostate_cancer_PRAD/TCGA-PRAD.GDC_phenotype.tsv', data.table = F)

# survival data
surv <- fread('/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/data/ucsc_xena/datasets/GDC_TCGA_Prostate_cancer_PRAD/TCGA-PRAD.survival.tsv', data.table = F)
...
prad <- merge(pheno, surv, by.x = 'submitter_id.samples', by.y='sample')
prad <- merge(prad, temp, by.x = 'submitter_id.samples', by.y=0)
...
prad <- merge(prad, epic$cellFractions, by.x = 'submitter_id.samples', by.y = 0)

# Order dataframe
prad %<>% set_rownames(prad$Row.names) %>%
...
table(prad$sample_type.samples)

# arrange df to allow for easy duplicate removal
praduniq <- prad %>% 
  ## set sample type to be a factor to allow for sorting
  mutate(sample_type.samples = factor(prad$sample_type.samples, levels = c('Primary Tumor', 'Recurrent Tumor', 'Metastatic', 'Solid Tissue Normal'))) %>%
...
praduniq <- praduniq[!duplicated(praduniq$patient_id),]

# verify patients and ffpe are uniform
table(praduniq$is_ffpe.samples)
table(duplicated(praduniq$patient_id))

```



# General QC of prad cohort
...
set.seed(11)
prad$comp <- prad$sample_type.samples
dat <- prad %>% dplyr::select(comp, TSPAN6:ncol(prad))
...
calculate_signature_scores <- function(DF, UPGENES, DOWNGENES, METACOLS = 1:140){
...
cur %<>% dplyr::select(submitter_id.samples, ss.TotalScore, directional_Zsum) %>%
  merge(., praduniq, by = 'submitter_id.samples')
...
# Define comparison
prad$comp <- prad$sample_type.samples
dat <- prad %>% dplyr::filter(sample_type.samples %in% c('Primary Tumor', 'Solid Tissue Normal'))
...
# Define comparison
praduniq$comp <- as.factor(praduniq$gleason_score)
...
autoplot(km_fit,
         conf.int = F,
         main = '306gs quartiles (1st/4th)')

```
### 306 gene signature Kaplan meijer for OS - subset by gleason
```{r}
...
#########
dat <- cur %>% mutate(comp = gleason_score)
...
#########
# stratify by gleason score (9)
#########
dat <- cur %>% 
  dplyr::filter(gleason_score >= 9) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[3], 3, 2)) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[4], 4, comp)) %>%
  mutate(comp = ifelse(ss.TotalScore < quantile(ss.TotalScore)[2], 1, comp)) 
...
autoplot(km_fit,
         conf.int = F,
         main = 'Ferroptosis quartiles in gleason >=9')

#########
# stratify by gleason score (8)
#########
dat <- cur %>% 
  dplyr::filter(gleason_score >= 8) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[3], 3, 2)) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[4], 4, comp)) %>%
  mutate(comp = ifelse(ss.TotalScore < quantile(ss.TotalScore)[2], 1, comp)) 
#dat %<>% dplyr::filter(comp %in% c(1,4))
...
autoplot(km_fit,
         conf.int = F,
         main = 'Ferroptosis quartiles in gleason >= 8')
