...
Take genes with fold change > 1.5 in GSE1044462 and genes with large z scores across groups
```{r}

sig_24h_ref <- sig_24h %>%
  # GSE104462 fc > 1.5
  dplyr::filter(abs(GSE104462_LFC) > log2(1.5))
# calculate abs(sum of z scores)
sig_24h_ref %<>% mutate(sums = rowSums(sig_24h_ref %>% select(HA1E_5um:HA1E_10um) %>% abs()))
# filter for genes greater than the median of the sum of the z scores
sig_24h_ref %<>% filter(sums >= median(sig_24h_ref$sums))
sig_24h_ref %<>% dplyr::select(-sums)
...
temp <- sig_24h_ref 
colnames(temp)[4:16] <- paste0(colnames(sig_24h)[4:16], '_24h')
write.table(temp, '/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/out/gene_signatures/erastin_clue24h_gse104462_83genes.txt',
            row.names = T, col.names = T, sep = '\t', quote = F)
