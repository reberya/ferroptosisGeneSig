...
litGenes <- readxl::read_xls('/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/data/pmid_ 32760210/Table S1.xls')

table(litGenes$`Ferrotosis-related genes` %in% clue.erastin$pr_gene_symbol)
table(litGenes$`Ferrotosis-related genes` %in% rownames(sig_24h))
table(litGenes$`Ferrotosis-related genes` %in% rownames(sig_24h_sub))
...
litGenes <- readxl::read_xlsx('/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/data/doi_10.1016_j.omto.2021.02.11/mmc2.xlsx', skip = 1)

table(litGenes$Symbol %in% clue.erastin$pr_gene_symbol)
table(litGenes$Symbol %in% rownames(sig_24h))
table(litGenes$Symbol %in% rownames(sig_24h_sub))
...
# Basically same signature used for 3 other papers; 60 genes
litGenes <- readxl::read_xls('/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/data/pmid_ 32760210/Table S1.xls')
litGenes <- litGenes$`Ferrotosis-related genes`


# select genes
rna.other60 <- rna %>% dplyr::filter(rownames(.) %in% litGenes)
clue.erastin_6h_5um.other60 <- clue.erastin_6h_5um %>% dplyr::filter(rownames(.) %in% litGenes)
clue.erastin_24h_10um.other60 <- clue.erastin_24h_10um %>% dplyr::filter(rownames(.) %in% litGenes) 
clue.erastin_24h_5um.other60 <- clue.erastin_24h_5um %>% dplyr::filter(rownames(.) %in% litGenes)
...
# Systematic profiling of ferroptosis gene signatures predicts prognostic factors in esophageal squamous cell carcinoma (Lu et al 2021)
litGenes <- readxl::read_xlsx('/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/data/doi_10.1016_j.omto.2021.02.11/mmc2.xlsx', skip = 1)
litGenes <- litGenes$Symbol

# Remove IFNg and CDO1 because not present in any of the RNA-seq datasets
litGenes <- litGenes[!litGenes %in% c('IFNG', 'CDO1')]

# select genes
rna.other149 <- rna %>% dplyr::filter(rownames(.) %in% litGenes)
