...
sig_24h_sub <- rownames(sig_24h_merge_sub[c(row_order(H)[['5']],row_order(H)[['2']]),])
...
56 gene signature captures 6/37
```{r}
...
temp <- sig_24h_sub
colnames(temp)[4:16] <- paste0(colnames(sig_24h)[4:16], '_24h')
write.table(temp, '/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/out/gene_signatures/erastin_clue24h_gse104462_56genes.txt',
            row.names = T, col.names = T, sep = '\t', quote = F)
...
# remove lines with strong signal
temp <- c('T3M10_5um_6h', 'CORL23_5um_6h', 'SNU1040_5um_6h', 'CL34_5um_6h', 'NCIH1694_5um_6h')
sig_l1000_merge_sub <- sig_l1000_merge %>% select(-temp)

# Plot without strong cell lines
set.seed(11)
H <- Heatmap(sig_l1000_merge_sub,
        show_column_dend = F,
        show_row_dend = F,
        show_row_names = F,
        column_names_gp = gpar(fontsize = 7),
...
# Take cluster 1 and 5
ComplexHeatmap::row_order(H)
sig_l1000_sub <- rownames(sig_l1000_merge_sub[c(row_order(H)[['4']],row_order(H)[['1']]),])
sig_l1000_sub <- sig_l1000 %>% dplyr::filter(rownames(.) %in% sig_l1000_sub)
...
# select genes
rna.sig_l1000_sub <- rna %>% dplyr::filter(rownames(.) %in% rownames(sig_l1000_sub))
clue.erastin_6h_5um.sig_l1000_sub <- clue.erastin_6h_5um %>% dplyr::filter(rownames(.) %in% rownames(sig_l1000_sub))
clue.erastin_24h_10um.sig_l1000_sub <- clue.erastin_24h_10um %>% dplyr::filter(rownames(.) %in% rownames(sig_l1000_sub)) 
clue.erastin_24h_5um.sig_l1000_sub <- clue.erastin_24h_5um %>% dplyr::filter(rownames(.) %in% rownames(sig_l1000_sub))

# order
rna.sig_l1000_sub <- rna.sig_l1000_sub[order(match(rownames(rna.sig_l1000_sub),rownames(sig_l1000_sub))),]
clue.erastin_6h_5um.sig_l1000_sub <- clue.erastin_6h_5um.sig_l1000_sub[order(match(rownames(clue.erastin_6h_5um.sig_l1000_sub),rownames(sig_l1000_sub))),]
clue.erastin_24h_10um.sig_l1000_sub <- clue.erastin_24h_10um.sig_l1000_sub[order(match(rownames(clue.erastin_24h_10um.sig_l1000_sub),rownames(sig_l1000_sub))),]
clue.erastin_24h_5um.sig_l1000_sub <- clue.erastin_24h_5um.sig_l1000_sub[order(match(rownames(clue.erastin_24h_5um.sig_l1000_sub),rownames(sig_l1000_sub))),]


set.seed(11)
Heatmap(rna.sig_l1000_sub[,1:3],
        show_column_dend = F,
        show_row_dend = F,
        show_row_names = F,
        column_names_gp = gpar(fontsize = 7),
...
Heatmap(clue.erastin_6h_5um.sig_l1000_sub,
        show_column_dend = F,
        show_row_dend = F,
        show_row_names = F,
        row_names_gp = gpar(fontsize = 5),
...
Heatmap(clue.erastin_24h_5um.sig_l1000_sub,
        show_column_dend = F,
        show_row_dend = F,
        show_row_names = F,
        column_names_gp = gpar(fontsize = 7),
  ...
  Heatmap(clue.erastin_24h_10um.sig_l1000_sub,
        show_column_dend = F,
        show_row_dend = F,
        show_row_names = T,
        column_names_gp = gpar(fontsize = 7),
...
temp <- sig_l1000 
colnames(temp)[4:16] <- paste0(colnames(sig_24h)[4:16], '_24h')
write.table(temp, '/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/out/gene_signatures/erastin_clue24h_gse104462_117genes.txt',
            row.names = T, col.names = T, sep = '\t', quote = F)

temp <- sig_l1000_sub
colnames(temp)[4:16] <- paste0(colnames(sig_24h)[4:16], '_24h')
write.table(temp, '/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/out/gene_signatures/erastin_clue24h_gse104462_36genes.txt',
            row.names = T, col.names = T, sep = '\t', quote = F)
...
temp <- getBM(attributes=c('external_gene_name', 'entrezgene_id'),
...
# gene universe
universe <- merge(clue.erastin, temp, by.x = 'pr_gene_symbol', by.y='external_gene_name')
universe <- unique(unlist(universe$entrezgene_id))

sig_24h.entrez <- merge(sig_24h, temp, by.x = 0, by.y='external_gene_name')


params <- new('GOHyperGParams',
              geneIds = sig_24h.entrez$entrezgene_id,
