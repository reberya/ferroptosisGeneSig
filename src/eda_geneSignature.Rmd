...
clue.erastin_6h_5um.resp <- clue.erastin_6h_5um %>% 
  set_rownames(clue.erastin_6h_5um$pr_gene_symbol) %>%
  dplyr::select(responsive) 
clue.erastin_6h_5um.resp %<>% dplyr::filter(rownames(.) %in% rownames(erastin)) 

# make rownames in same order as erastin.plot
clue.erastin_6h_5um.resp <- clue.erastin_6h_5um.resp[order(match(rownames(clue.erastin_6h_5um.resp),rownames(erastin.plot))),]
...
Heatmap(clue.erastin_6h_5um.resp,
        show_column_dend = F,
        show_row_dend = F,
        show_row_names = T,
        row_names_gp = gpar(fontsize = 5),
