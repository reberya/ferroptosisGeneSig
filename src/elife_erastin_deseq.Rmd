...
## sum of expression for controls/erastin used for filtering 
counts$sums <- rowSums(counts[, c('dmso_1', 'dmso_2', 'erastin_1', 'erastin_2')])
## expression >4 across 4 samples
counts %<>% dplyr::filter(sums >= 10)
## rownames
counts %<>% set_rownames(counts$gene) %>% select(-gene, -sums)
...
temp <- res %>% dplyr::filter(abs(log2FoldChange) >=1)
write.table(temp, '/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/int/elife2014_erastin/rna_seq/sig_genes_log2fc.txt', quote = F, sep = '\t', row.names = T, col.names = T)
