...
# save 
write.table(prop, '/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/out/EPIC/NBL/cell_proportions/nbl_epic_proportions_all.txt',
            row.names = F, col.names = T, sep = '\t', quote = F)
write.table(prop.sub, '/mctp/share/users/rebernrj/projects/ferroptosisGeneSig/out/EPIC/NBL/cell_proportions/nbl_epic_proportions_sub.txt',
            row.names = F, col.names = T, sep = '\t', quote = F)
...
make_plots <- function(DF = prop.sub,
  ...
  return(list(b0,b1,b2,b3,b4,b5,b6,b7))
...
# Define comparison
prop$comp <- rep('all', length(prop$rlib))
...
#### Pre/post dinutuxumab 
```{r}

# Define comparison
prop$comp <- prop$dinu_treat_prepost
dat <- prop %>% dplyr::filter(comp %in% c('postDIN', 'preDIN'))
...
make_plots(DF = dat,
           ...
           TIMEPOINT = 'all', 
           STAT_TEST = 't.test',
           COMPARISON_TITLE = 'Dinutuxumab Pre/Post')

```

#### Event-free survival from time of Dinutuximab txt split by mean
```{r}

# Quantiles of EFS time
q <- quantile(prop$dinu_efstime, na.rm = T)

# Define comparison
prop$comp <- ifelse(prop$dinu_efstime >= q[3], 'Long EFS', 'Short EFS')
dat <- prop %>% dplyr::filter(!is.na(dinu_efstime))
...
make_plots(DF = dat,
           ...
           TIMEPOINT = 'all', 
           STAT_TEST = 't.test',
           COMPARISON_TITLE = 'EFS split by mean(EFS)')

```
#### Event-free survival from time of Dinutuximab txt split into quartiles
```{r}
...
# Define comparison
prop$comp <- ifelse(prop$dinu_efstime >= q[3], '3', '2')
prop$comp <- ifelse(prop$dinu_efstime >= q[4], '4', prop$comp)
prop$comp <- ifelse(prop$dinu_efstime < q[2], '1', prop$comp)
dat <- prop %>% dplyr::filter(!is.na(dinu_efstime))
...
make_plots(DF = dat,
           ...
           TIMEPOINT = 'all', 
           STAT_TEST = 'anova',
           COMPARISON_TITLE = 'EFS split by quartiles')
...
# Quantiles of EFS time
q <- quantile(prop$dinu_stime, na.rm = T)

# Define comparison
prop$comp <- ifelse(prop$dinu_stime >= q[3], 'Long OS', 'Short OS')
dat <- prop %>% dplyr::filter(!is.na(dinu_stime))
...
make_plots(DF = dat,
           ...
           TIMEPOINT = 'all', 
           STAT_TEST = 't.test',
           COMPARISON_TITLE = 'OS split by mean(EFS)')

```
#### Overall survival from time of Dinutuximab txt split into quartiles
```{r}
...
# Define comparison
prop$comp <- ifelse(prop$dinu_stime >= q[3], '3', '2')
prop$comp <- ifelse(prop$dinu_stime >= q[4], '4', prop$comp)
prop$comp <- ifelse(prop$dinu_stime < q[2], '1', prop$comp)
dat <- prop %>% dplyr::filter(!is.na(dinu_stime))
...
make_plots(DF = dat,
           ...
           TIMEPOINT = 'all', 
           STAT_TEST = 'anova',
           COMPARISON_TITLE = 'OS split by quartiles')
