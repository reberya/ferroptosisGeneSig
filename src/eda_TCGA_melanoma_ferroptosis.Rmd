...
### 306 gene signature Kaplan meijer for OS - breslow depth and ulceration
```{r}
...
# quartile
dat <- cur %>%
  mutate(comp = ifelse(breslow_depth_value >= quantile(breslow_depth_value, na.rm = T)[3], 3, 2)) %>%
  mutate(comp = ifelse(breslow_depth_value >= quantile(breslow_depth_value, na.rm = T)[4], 4, comp)) %>%
  mutate(comp = ifelse(breslow_depth_value < quantile(breslow_depth_value, na.rm = T)[2], 1, comp)) 
# plot Kaplan meijer
km_fit <- survfit(Surv(OS.time, rep(1, length(dat[,1]))) ~ comp, data=dat)
summary(km_fit, times = c(1,30,60,90*(1:10)))
autoplot(km_fit,
         conf.int = F,
         main = 'OS by breslow depth quartile')

###############
# Ulceration
################
# quartile
dat <- cur %>% mutate(comp = melanoma_ulceration_indicator) %>%
  dplyr::filter(comp %in% c('NO', 'YES'))
# plot Kaplan meijer
km_fit <- survfit(Surv(OS.time, rep(1, length(dat[,1]))) ~ comp, data=dat)
summary(km_fit, times = c(1,30,60,90*(1:10)))
autoplot(km_fit,
         conf.int = F,
         main = 'OS by ulceration stauts')
...
#################
# 306gs all samples all quartile
#################
...
autoplot(km_fit,
         conf.int = F,
         main = 'OS by 306gs quartile')

#################
# 306gs all samples 1&4 quartile
#################
# stratify by quartile 
dat <- cur %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[3], 3, 2)) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[4], 4, comp)) %>%
  mutate(comp = ifelse(ss.TotalScore < quantile(ss.TotalScore)[2], 1, comp)) 
dat %<>% dplyr::filter(comp %in% c(1,4))
# plot Kaplan meijer
km_fit <- survfit(Surv(OS.time, rep(1, length(dat[,1]))) ~ comp, data=dat)
summary(km_fit, times = c(1,30,60,90*(1:10)))
autoplot(km_fit,
         conf.int = T,
         main = 'OS by 306gs 1st/4th quartile')

#################
# 306gs highest breslow 1&4 quartile
#################
# stratify by quartile 
dat <- cur %>%
  dplyr::filter(breslow_depth_value >= quantile(breslow_depth_value, na.rm = T)[4]) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[3], 3, 2)) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[4], 4, comp)) %>%
  mutate(comp = ifelse(ss.TotalScore < quantile(ss.TotalScore)[2], 1, comp)) 
dat %<>% dplyr::filter(comp %in% c(1,4))
...
autoplot(km_fit,
         conf.int = F,
         main = 'OS by 1st/4th quartile 306gs for 4th quartile breslow')
...
# stratify by quartile 
dat <- cur %>%
  dplyr::filter(melanoma_ulceration_indicator == 'YES') %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[3], 3, 2)) %>%
  mutate(comp = ifelse(ss.TotalScore >= quantile(ss.TotalScore)[4], 4, comp)) %>%
  mutate(comp = ifelse(ss.TotalScore < quantile(ss.TotalScore)[2], 1, comp)) 
dat %<>% dplyr::filter(comp %in% c(1,4))
...
autoplot(km_fit,
         conf.int = F,
         main = 'OS by 1st/4th quartile 306gs for Ulceration = Y')
...
dat <- skcm %>% dplyr::filter(comp %in% c('Primary Tumor', 'Regional Lymph Node'))
